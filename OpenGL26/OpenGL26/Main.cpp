...
void PrintAttribute(FbxNodeAttribute*pAttribute, FbxNode*pNode, FbxScene*pScene, FbxTakeInfo*pTakeInfo)
{
	...
	if(typeName ==  GetAttributeTypeName(fbxsdk::FbxNodeAttribute::eMesh))
	{
		...
		//for(int asdf = 0; asdf < numIndices; asdf += 3)
			//printf("%d: INDICE %d - %d - %d\n", asdf, indices[asdf+0], indices[asdf+1], indices[asdf+2]);
		...
		if(normalEl)
		{
			...
			{
				printf("NORMALCONTROLPOINT!\n");
			}
			else if(normalEl->GetMappingMode() == FbxGeometryElement::eByPolygonVertex)
			{
				printf("NORMALPOLYGONVERTEX!\n");
			}
		...
		{
			printf("NORMALTEST X: %f Y: %f Z: %f\n", blaaargh[aaa*3+0], blaaargh[aaa*3+1], blaaargh[aaa*3+2]);
		}
		_getch();
		*/
		...
		*/
		...
		/*
		for(int derp = 0; derp < 50; derp++)
		{
			printf("UV U: %f V: %f\n", myUVmap[derp*2+0], myUVmap[derp*2+1]);
		}
		_getch();
...
int main()
{
	...
	for(int i = 0; i <= numIndices - 3; i+=3)
	{
		for(int j = i; j <= i + 2; j++)
		{
			...
			printf("Vertex X: %f Y: %f Z: %f\n", vertices[fbxindices[j]].x, vertices[fbxindices[j]].y, vertices[fbxindices[j]].z);
			printf("INDICE: %d iloop: %d jloop: %d - POLYVERT: %d\n", fbxindices[j], i, j, polyvert);
			myVertices[j*3+0] = vertices[fbxindices[j]].x;
			myVertices[j*3+1] = vertices[fbxindices[j]].y;
			myVertices[j*3+2] = vertices[fbxindices[j]].z;
			polyvert++;
		}
		//printf("BREAK!\n");
		//_getch();
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(myUVmap), myUVmap, GL_STATIC_DRAW);
	...
	*/
	...
	for(;;)
	{
		...
		*/
		...
		//glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, sizeof(GLfloat)*12, (void*)0);
		...
		//glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, sizeof(GLfloat)*6, (void*)0);
		...
		glDrawArrays(GL_TRIANGLES, 0, numIndices * sizeof(glm::vec3));
		//printf("DERP: %d\n", numIndices * sizeof(glm::vec3));
